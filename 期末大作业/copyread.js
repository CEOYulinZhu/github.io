    ...
    if (questionType === 'single' || questionType === 'multiple') {
        ...
        return optionLetters.map((letter, _index) => `
            <div class="option-item" id="option-item">
                   ...
                   <input class="optiontext" type="text" placeholder="输入选项内容">
...
function addOption(button,tempCurrentQuestionType) {
    ...
    optionLabel.innerHTML = `
        ...
        <input type="text" placeholder="输入选项内容" class="optiontext">
        <button onclick="deleteOption(this)">删除选项</button>
    `;
    if (optionIndex < 2) {
...
surveys.forEach((surveyData, index) => {
    if(index===indexnumber){
        ...
        //本地的截止日期
        let deadline = `${surveyData.deadline}`
...
function saveSurvey() {
   ...
   surveys.splice(indexnumber,1,surveyData);
   localStorage.setItem('surveys', JSON.stringify(surveys));
